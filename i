...
SHELL_FUNC='function f() { case "$1" in ""|-*|index|list|add|remove|boost|prune|top|help) "$HOME/.local/bin/f" "$@"; return;; esac; local dir; dir=$("$HOME/.local/bin/f" "$@"); [ -d "$dir" ] && cd "$dir" && "$HOME/.local/bin/f" boost "$dir" 2>/dev/null; }'
